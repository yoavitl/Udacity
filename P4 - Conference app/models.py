...
class Session(ndb.Model):
    ...
    typeofs     = ndb.StringProperty()
    date        = ndb.DateProperty()
    startTime   = ndb.FloatProperty()
    # assign the session to the conf
...
class SessionForm(messages.Message):
    ...
    typeofs     = messages.StringField(5)
...
class Speaker(ndb.Model):
    ...
    # Entity for speakers, could be more implemented
    name        = ndb.StringProperty(required=True)
    expertise   = ndb.StringProperty(repeated=True)
    lecturs     = ndb.StringProperty(repeated=True)
...
class SpeakerForm(messages.Message):
    ...
    name        = messages.StringField(1)
    expertise   = messages.StringField(2, repeated=True)
